...
bot = telebot.TeleBot("5030984361:AAEgqPlES5ORzFcc8ecnPkahZzIL4DoxONM")
```
```python
...
def i(message: telebot.types.Message):
    print(message.photo)
    bot.send_message(message.chat.id, "Zaebis")
```   
```python
...
def u(message: telebot.types.Message):
    print(message.voice)
    bot.send_message(message.chat.id, "Заебали ваши голосовые сообщения!")
```    
```python
...
def u(message: telebot.types.Message):
    ...
    bot.send_message(message.chat.id, "Больше бюрократии!")
```    
```python
